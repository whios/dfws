...
async function writeAuditLog(actorId, target, previousName, nextName, previousBrand, nextBrand) {
  try {
    const nameChanged = previousName !== nextName;
    const brandChanged = previousBrand !== nextBrand;
    await supabaseFetch('/rest/v1/operation_audit_logs', {
      method: 'POST', headers: { ...serviceHeaders(), Prefer: 'return=minimal' },
      body: JSON.stringify({ actor_id: actorId, action: nameChanged && brandChanged ? '修改人员姓名与品牌' : brandChanged ? '调整人员品牌' : '修改人员姓名', target_type: 'profile', target_id: target.id, target_name: nextName, brand: nextBrand || null, details: { previousName, previousBrand, email: target.email } })
    });
  } catch (error) {
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (target.partner_id) {
      ...
      previousBrand = partner.brand;
      nextBrand = String(requestedBrand || partner.brand).trim();
      if (!allowedBrands.has(nextBrand)) return reply(response, 400, { error: '请选择有效的所属品牌。' });
      const sameName = await supabaseFetch(`/rest/v1/partners?owner_name=eq.${encodeURIComponent(nextName)}&brand=eq.${encodeURIComponent(nextBrand)}&department=eq.${encodeURIComponent(partner.department)}&id=neq.${encodeURIComponent(partner.id)}&select=id`, { headers: serviceHeaders() });
      if (sameName?.length) return reply(response, 409, { error: '同品牌、同部门已存在同名伙伴档案。请使用可区分的姓名后再保存。' });
      await supabaseFetch(`/rest/v1/partners?id=eq.${encodeURIComponent(partner.id)}`, { method: 'PATCH', headers: { ...serviceHeaders(), Prefer: 'return=minimal' }, body: JSON.stringify({ owner_name: nextName, brand: nextBrand }) });
      await Promise.all([
        ...
        supabaseFetch(`/rest/v1/assets?partner_id=eq.${encodeURIComponent(partner.id)}`, { method: 'PATCH', headers: { ...serviceHeaders(), Prefer: 'return=minimal' }, body: JSON.stringify({ owner_name: nextName, brand: nextBrand }) })
      ]);
    } else if (requestedBrand) {
      return reply(response, 409, { error: '该账号尚未绑定伙伴档案，请先绑定伙伴后再调整品牌。' });
    }
    if (target.display_name === nextName && previousBrand === nextBrand) return reply(response, 200, { ok: true, unchanged: true, message: '姓名和品牌均未变化。' });
    ...
    await writeAuditLog(currentUser.id, target, target.display_name || '', nextName, previousBrand, nextBrand);
    return reply(response, 200, { ok: true, profile: updated[0], message: target.partner_id ? '姓名和品牌已同步到账号、伙伴档案与资产归属；组织部门归属保持不变。' : '账号显示名已更新。' });
  } catch (error) {

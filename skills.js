...
function skills() {
  ...
  const render = () => {
    ...
    $('#skill-card-grid').innerHTML = filtered.map((resource) => {
      ...
      const controls = canReview ? `<textarea class="skill-card-note" data-skill-note="${resource.id}" placeholder="审核备注，伙伴被退回后可见">${esc(resource.review_note || '')}</textarea><div class="skill-card-actions"><select data-skill-status="${resource.id}">${statusOptions.map(([value, label]) => `<option value="${value}" ${resource.status === value ? 'selected' : ''}>${label}</option>`).join('')}</select><span style="flex:1"></span><button class="button secondary" data-download-skill="${resource.id}">下载文件</button>${showcasePath ? `<button class="button secondary" data-download-showcase="${resource.id}">下载展示附件</button>` : ''}<button class="button secondary" data-edit-skill="${resource.id}">编辑</button><button class="button primary" data-save-skill="${resource.id}">保存</button><button class="button secondary danger-action" data-delete-skill="${resource.id}">删除成果</button></div>` : '<div class="skill-card-readonly">领导只读</div>';
    ...
    $$('.skill-library-card').forEach((card, index) => {
      ...
      if (!canReview || resource?.status !== 'published') return;
  ...
  const load = async () => {
    try {
      const [data, partnerRows] = await Promise.all([window.DfwsCloud.listSkillResources(canReview), canReview ? window.DfwsCloud.listSkillPartners() : Promise.resolve([])]);

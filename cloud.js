...
(function () {
  ...
  async function listOperationAuditLogs() {
    if (!personnelAdmin()) throw new Error('仅 AI 应用官和负责人可以查看操作日志。');
    const { data, error } = await client.rpc('list_operation_audit_logs', { max_rows: 100 });
    if (error) throw error;
    return data || [];
  }
  ...
  async function editPerson(id, values) {
    ...
    if (!values || typeof values !== 'object' || typeof values.displayName !== 'string') throw new Error('请填写人员姓名。');
    const { data: { session } } = await client.auth.getSession();
    if (!session?.access_token) throw new Error('登录状态已失效，请重新登录。');
  ...
  async function batchInviteMembers(records) {
    ...
    if (!Array.isArray(records) || !records.length) throw new Error('没有可开通的人员。');
    const { data: { session } } = await client.auth.getSession();
    if (!session?.access_token) throw new Error('登录状态已失效，请重新登录。');
    const response = await fetch('/api/staff/batch-invite-members', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
